        ...
        for (int i = 0; i < 10; i++) {//尝试1000次
            executorService.execute(() -> {
                ...
                try {
                    ...
                    // 没有秒完
                    if (valInteger < 1) {//redisKey
                        Transaction tx = jedis1.multi();//开启事务
                        tx.incr(redisKey);//自增
                        List list = tx.exec();//提交事务，如果返回nil则说明执行失败，因为我watch了的，只要执行失败，则
                        // 进来发现东西还有，秒杀成功
                        if (list != null && list.size() > 0) {
                            System.out.println("用户：" + userInfo + "，投标成功！当前成功人数：" + (valInteger + 1) + ThreadLocal.class.getSimpleName());
                        } else {//执行结果不是OK，说明被修改了，被别人抢了
                            System.out.println("用户：" + userInfo + "，投标失败");
                        }
                    } else {//东西秒完了
                        System.out.println("已经有1人投标成功，投标结束");
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                } finally {//关闭redis
